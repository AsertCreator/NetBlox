...
namespace Qu3e
{
	public struct Mat3
	{
		...
		public Vec3 ToEuler()
		{
			...
			if (ez.x != 1 && ez.x != -1)
			{
				double pitch_1 = -1 * Math.Asin(ez.x);
				double roll_1 = Math.Atan2(ez.y / Math.Cos(pitch_1), ez.z / Math.Cos(pitch_1));
				double yaw_1 = Math.Atan2(ey.x / Math.Cos(pitch_1), ex.x / Math.Cos(pitch_1));
				pitch = pitch_1;
				roll = roll_1;
				yaw = yaw_1;
			}
			else
			{
				...
				{
					pitch = Math.PI / 2;
					roll = yaw + Math.Atan2(ex.y, ex.z);
				}
				else 
				{
					pitch = -Math.PI / 2;
					roll = -1 * yaw + Math.Atan2(-1 * ex.y, -1 * ex.z); 
				}
			}
