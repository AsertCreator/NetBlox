...
namespace Qu3e
{
	...
	public class Body
	{
		...
		public Box AddBox(BoxDef def)
		{
			...
			Boxes.Add(box);
			box.ComputeAABB(Tx, out AABB aabb);
		...
		public void ApplyTorque(Vec3 torque) => Torque += torque;
		...
		public bool IsAwake() => (Flags & eAwake) > 0;

		public double GetGravityScale() => GravityScale;
		public void SetGravityScale(double scale) => GravityScale = scale;
		public Vec3 GetLocalPoint(Vec3 p) => Transform.MulT(Tx, p);
		public Vec3 GetLocalVector(Vec3 v) => Transform.MulT(Tx.rotation, v);
		public Vec3 GetWorldPoint(Vec3 p) => Transform.Mul(Tx, p);
		public Vec3 GetWorldVector(Vec3 v) => Transform.Mul(Tx.rotation, v);
		public Vec3 GetLinearVelocity() => LinearVelocity;
		...
		public Vec3 GetAngularVelocity() => AngularVelocity;
		...
		public Transform GetTransform() => Tx;
		public BodyFlags GetFlags() => Flags;
		public void SetLayers(int layers) => Layers = layers;
		public int GetLayers() => Layers;
		public Quaternion GetQuaternion() => Q;
		public object GetUserData() => UserData;

		public void SetLinearDamping(double damping) => LinearDamping = damping;
		public double GetLinearDamping(double damping) => LinearDamping;
		public void SetAngularDamping(double damping) => AngularDamping = damping;
		public double GetAngularDamping(double damping) => AngularDamping;
		...
		public void Dump(StringBuilder file, int index)
		{
			...
			Q.ToAxisAngle(out Vec3 axis, out double angle);
		...
		public double GetInvMass() => InvMass;
		...
		private void CalculateMassData()
		{
			...
			foreach (var box in Boxes)
			{
				...
				box.ComputeMass(out MassData md);
				mass += md.mass;
				inertia += md.inertia;
