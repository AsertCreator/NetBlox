...
namespace NetBlox.Instances.GUIs
{
	...
	public class TextLabel : GuiObject
	{
		...
		public TextLabel(GameManager ins) : base(ins) => _font = GameManager.RenderManager.MainFont;
		...
		public override void RenderGUI(Vector2 cp, Vector2 cs)
		{
			if (Visible)
			{
				...
				if (!LeftAligned)
				{
					...
					Raylib.DrawTextEx(_font.SpriteFont, Text, p + new Vector2(0, s.Y / 2 - FontSize / 2), FontSize, 0, TextColor3);
			}
